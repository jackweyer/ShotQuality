...
Original file is located at
    https://colab.research.google.com/drive/1O3GxtM_hYhGLKSAzgr6ZEtZQl0FZ4Kgd

## Getting players at the start of a period
...
#def play_by_play_url(game_id):
 #   return "https://stats.nba.com/stats/playbyplayv2/?gameId=00{0}&startPeriod=0&endPeriod=14".format(game_id)
## add in 00's to match the formatting

def advanced_boxscore_url(game_id, start, end):
    return "https://stats.nba.com/stats/boxscoreadvancedv2/?gameId=00{0}&startPeriod=0&endPeriod=14&startRange={1}&endRange={2}&rangeType=2".format(game_id, start, end)
...
df = pd.read_csv("nba_pbp.csv")
games = df['idGame'].unique()
fulldf = pd.DataFrame()
urllib3.disable_warnings()

for game_id in games:
#game_id = "22100001" ###add 00 to start
  ...
  substitutionsOnly = df[df.idGame ==game_id][df.numberEventMessageType==8][['numberPeriod', 'numberEvent', 'idPlayerNBA1', 'idPlayerNBA2']]
  ...
  out['gameID'] = game_id
  fulldf = fulldf.append(out)
  #print(out)
  #time.sleep(1)

fulldf.to_csv("QuarterStarters.csv", index=False)